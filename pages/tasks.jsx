...
export default function Home() {
  const [tasks, setTasks] = useState([
    {
      name: "read and annotate the source",
      time: 60,
      date: [9, 12, 22],
      key: [1],
    },
    ...
    <div>
      ...
      >
        <Header
          ...
          rightLink={"schedule"}
        />
        <MobileHeader name={"tasks"} color="blue" topLeftButton={"+"} leftButtonLink=""/>

        <Day day={"today"} />
        {tasks.map((task) => task.date[1] == 12 && <Task name={task.name} />)}
        <NewTask setTask={setTasks} tasks={tasks} day={[9, 12, 22]} />
        <Day day={"tommorow"} />
        {tasks.map((task) => task.date[1] == 13 && <Task name={task.name} />)}
        <NewTask setTask={setTasks} tasks={tasks} day={[9, 13, 22]} />
        <Day day={"9/14"} />
        {tasks.map((task) => task.date[1] == 14 && <Task name={task.name} />)}
        <NewTask setTask={setTasks} tasks={tasks} day={[9, 14, 22]} />
        <Day day={"9/15"} />
        {tasks.map((task) => task.date[1] == 15 && <Task name={task.name} />)}
